...
class VolumeGui(FrameMaestro):
    ...
    def loadVolume(self):
        ...
        if (self.volume.publisherId!=0):
            editorial = self.session.query(Publisher).get(self.volume.publisherId)
            self.entradaEditorial.insert(0,editorial.name)
