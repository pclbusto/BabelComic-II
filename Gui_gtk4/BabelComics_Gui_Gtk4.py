...
import Entidades_Gtk4.Init
from Entidades_Gtk4.Agrupado_Entidades import Comicbook, Publisher, Volume, Comicbook_Info, Arcos_Argumentales_Comics_Reference, Comicbook_Detail
...
import os.path, math, threading, subprocess
from PIL import ImageFile
from rarfile import NotRarFile, BadRarFile

icons = ["edit-cut", "edit-paste", "edit-copy"]

class BabelComics_main_gtk(Gtk.ApplicationWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.session = Entidades_Gtk4.Init.Session()

        # self.listaEditoriales = self.session.query(Publisher).all()
        title = Gtk.Label()
        title.set_text("Babel Comics")
        ...
        self.app_icon = Pixbuf.new_from_file_at_scale('../iconos/iconoBabelComics-buuf.png', 32, 32,True)
        self.set_default_icon_list([self.app_icon])
        # self.child_set()
        #
        # self.load_setup()
        #
...
def activate(app):
    win = BabelComics_main_gtk(application=app)
    win.show()


if __name__ == "__main__":
    '''
    Se tomó esta pagina para ver que id calcular
    https://wiki.gnome.org/HowDoI/ChooseApplicationID
    '''
    app = Gtk.Application(application_id="github.com.BabelComics")
